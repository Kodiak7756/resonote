...
- **Instruments** — the display above the pedals can be a guitar neck (any tuning), a piano, a **Lumatone** hex board (Wicki-Hayden or Bosanquet layout — the same interval is always the same step, so a shape learned in one key is the same shape in every key), **Vocals** (a pitch ladder with a live trace of what you sing, target rungs for whatever a pedal lights, and a find-my-range tool), or **Drums** (a full kit — kick, snare, hi-hat, two rack toms, floor tom, two crashes, ride and a clap pad — that you click, tap with several fingers at once, or play from the keyboard: K kick · S snare · H / O hi-hat · 1 2 3 toms · C V crashes · R ride · P pad).
- **Backing Track** (a STUDIO pedal, 🥁) — the sequencer the drum kit writes into. **● Rec** lays down what you play on the kit while the beat loops, snapped to the nearest 16th (Shift-hit erases); **✎ Step** picks a step and toggles whatever pieces you hit onto it; beats can be 1, 2 or 4 bars (room for a fill and a crash), and **↶ Undo** takes back a whole recording pass. The kit lights up as the beat plays.
